  ...
  def init_embeddings(self, hparams, scope):
    ...
    self.embedding_encoder, self.embedding_decoder = (
        model_helper.create_emb_for_encoder_and_decoder(
            ...
            src_embed_size=hparams.embed_size,
            tgt_embed_size=hparams.embed_size,
...
class ModelNt(BaseModel): 
  ...
  def __init__(self,
    ...
    """
    assert isinstance(iterator, iterator_utils.BatchedInputNt)
    self.iterator = iterator
    self.mode = mode
    self.src_vocab_table = source_vocab_table
    self.tgt_vocab_table = target_vocab_table
    ...
    for i in range(hparams.num_traces):
      with tf.variable_scope("encoder%d" % i) as scope:
        ...
        enc_emb_inp = tf.nn.embedding_lookup(
            self.embedding_encoder, traces[i])

        if hparams.encoder_type == "uni":

          utils.print_out("  Encoder%d: num_layers = %d, num_residual_layers=%d" 
          ...
          enc_out, enc_state = tf.nn.dynamic_rnn(
              ...
              enc_emb_inp,
              dtype=dtype,
              sequence_length=iterator.traces_sequence_length[i],
              time_major=self.time_major,
              swap_memory=True)
          ...
          if num_bi_layers == 1:
            enc_state = bi_enc_state
          else:
            enc_state = []
            for layer_id in range(num_bi_layers):
              enc_state.append(bi_enc_state[0][layer_id])  # forward
              enc_state.append(bi_enc_state[1][layer_id])  # backward
...
###############################################################################################
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    # maximum_iteration: The maximum decoding steps.
    maximum_iterations = self._get_infer_maximum_iterations(
        hparams, iterator.traces_sequence_length[0])
