...
__all__ = ["BatchedInput", "BatchedInput2t" "get_iterator", "get_infer_iterator"]
...
def get_infer_iteratorNt(traces_dataset,
  ...
  traces = tuple(traces_and_lens[0:hparams.num_traces-1])
  trace_lens = tuple(traces_and_lens[hparams.num_traces:-1])
...
def get_iteratorNt(traces_dataset,
  ...
  traces = tuple(unpacked_iter[0:hparams.num_traces-1])
  tgt_input_ids = unpacked_iter[hparams.num_traces]
  tgt_output_ids = unpacked_iter[hparams.num_traces+1]
  trace_lens = tuple(unpacked_iter[hparams.num_trace+1:2*hparams.num_traces])
